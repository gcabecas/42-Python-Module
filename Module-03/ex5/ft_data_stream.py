...
def StreamProcessor():
    ...
    for event in events:
        ...
        if level >= 10:
            HighLevelPlayer += 1
        if eventname == 'item_found':
            Treasure += 1
        elif eventname == 'level_up':
            Levelup += 1
        player = event['player']
        level = event['data']['level']
        eventname = event['event_type']
        yield Num, HighLevelPlayer, Treasure, Levelup, player, level, eventname
...
def main():
    ...
    for Num, HighLevelPlayer, Treasure, Levelup, player, level, \
            event in processor:
        if i < 3:
    ...
    print(f"High-level players (10+): {HighLevelPlayer}")
